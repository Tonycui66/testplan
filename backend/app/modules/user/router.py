...
def token_response(user: User) -> TokenResponse:
    ...
    refresh_token = create_access_token(f"refresh:{user.id}", timedelta(days=7))
...
async def refresh(payload: RefreshRequest, db: AsyncSession = Depends(get_db)) -> TokenResponse:
    subject = decode_access_token(payload.refresh_token)
    if subject is None or not subject.startswith("refresh:"):
...
async def list_teams(page: int = 1, page_size: int = 20, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> dict[str, Any]:
    offset = (max(page, 1) - 1) * min(max(page_size, 1), 100)
    total = await db.scalar(select(func.count()).select_from(Team)) or 0
    teams = (await db.scalars(select(Team).order_by(Team.created_at.desc()).offset(offset).limit(page_size))).all()
...
async def get_team(team_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> dict[str, Any]:
...
async def update_team(team_id: UUID, payload: TeamUpdate, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> TeamResponse:
    team = await db.get(Team, team_id)
    if team is None:
        raise NotFoundError("Team not found")
...
async def delete_team(team_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> None:
    team = await db.get(Team, team_id)
    if team is None:
        raise NotFoundError("Team not found")
...
async def add_team_member(team_id: UUID, payload: TeamMemberCreate, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> TeamMemberCreate:
...
async def remove_team_member(team_id: UUID, user_id: UUID, db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> None:
...
async def list_admin_users(page: int = 1, search: str = "", db: AsyncSession = Depends(get_db), _: User = Depends(get_current_user)) -> dict[str, Any]:
    stmt = select(User)
    if search:
